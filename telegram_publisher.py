...
class TelegramPublisher:
        ...
        try:
            ...
            async with aiohttp.ClientSession(connector=connector) as session:
                ...
                for i, chunk in enumerate(chunks):
                    ...
                    if i == 0:
                        title = f"📊 Анализ квартир до 40 кв.м. - {datetime.now().strftime('%d.%m.%Y %H:%M')}\n\n"
                        
                        # Добавляем маркетинговый блок сразу после заголовка
                        market_block = "💸 Хотите купить недвижимость в Дубае выгодно и без посредников?\n\n"
                        ...
                        chunk = title + market_block + chunk
                    
                    # Для последнего чанка добавляем хэштеги
                    if i == len(chunks) - 1:
                        # Добавляем хэштеги
                        chunk = chunk + "\n\n#недвижимость #анализ #инвестиции"
                        chunk = chunk + "\n\n#Дубай #Недвижимость #ДешевыеКвартиры #Инвестиции #МаленькиеКвартиры"
                    
                    # Проверка длины каждого чанка перед отправкой
                    if len(chunk) > 4000:
                        logger.warning(f"Чанк {i+1}/{len(chunks)} слишком длинный ({len(chunk)} символов), обрезаем до 4000 символов")
                        chunk = chunk[:3997] + "..."
    ...
    async def publish_analysis(self):
        ...
        try:
            ...
            # Вставляем маркетинговый блок после первой строки анализа
            ...
            if len(lines) > 1:
                analysis = lines[0] + market_block + lines[1]
            else:
                analysis = lines[0] + market_block
            
            # Сохраняем измененный анализ в файл
            current_datetime = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_file = f"reports/cheapest_apartments_fixed_{current_datetime}.txt"
...
async def main():
    """Основная функция"""
    logger.info("Запуск скрипта публикации анализа квартир в Telegram")
